...
void orderManager(struct order *current);
...
{ 
	...
	// vesszuk ki a sorbol 
     status = msgrcv(uzenetsor, &uz, 1024, 5, 0 ); //msgrcv(mit, &hova, memóriaszelet, melyik típusú üzenetet az üzenetsorból //3-mtype mez , 0. elem );
     if ( status < 0 ) 
          perror("msgsnd"); 
     else
...
//send orders to chile, and optimaze them
void orderManager(struct order *current){
  optimize(current);
  int idx = 0;
  printf("\n \t *MANAGE JOBS* \n");
  ...
  for (idx = 0; idx < nrOfJobs; ++idx)
  {
    checkValues(&currentJob[idx]);
  }
  
  printf("number of jobs: %d\n", nrOfJobs);
  printf("sending job to child...\n");

  /*pid_t child;
  int uzenetsor, szuloVGyerek;
  key_t kulcs;
  kulcs = ftok(argv[0],1);*/

  int pipefc[2]; // unnamed pipe for send data back from child
        pid_t pid2;
        int done = 0;
  ...
  int pipefd[2]; // unnamed pipe file descriptor array
        ...
        struct order tmp_current/* = malloc(sizeof(struct order))*/;
        int current_nr;
    
    if (pipe(pipefd) == -1)     // itt jön létre a csővezeték
    {
      perror("Hiba a pipefd nyitaskor!");
      exit(EXIT_FAILURE);
    }

    if (pipe(pipefc) == -1)     // itt jön létre a csővezeték
    {
      perror("Hiba a pipefc nyitaskor!");
      exit(EXIT_FAILURE);
    }
  ...
  for (idx = 0; idx < nrOfJobs; ++idx)
  {
    ...
    if (pid1 == -1)
    {
      perror("PID1 Fork hiba");
      exit(EXIT_FAILURE);
    }

    if (pid2 == -1)
    {
      perror("PID2 Fork hiba");
      exit(EXIT_FAILURE);
    }
    ...
    {                // child process
      ...
      close(pipefc[0]); //write to second
         close(pipefd[1]);  //Usually we close the unused write end
         printf("Gyerek elkezdi olvasni a csobol az adatokat!\n");
             //read(pipefd[0],sz,sizeof(sz)); // reading max 100 chars
             //read(pipefd[0],tmp_current,sizeof(tmp_current)); // reading max 100 chars
             if(read(pipefd[0],current_nr,sizeof(current_nr)) >0){
              checkValues(&currentJob[current_nr]);
              printf("Gyerek olvasta uzenet ");
             }else{
              printf("Gyerek nem tud olvasni!\n");
              exit(EXIT_FAILURE);
              exit(EXIT_FAILURE);
             } //
         ...
         if(write(pipefc[1], done, sizeof(done))>0){
          printf("Gyerek valaszolt pipefc-n\n");
         }else{
          printf("gyerek válasz hiba\n");
         }
      close(pipefd[0]); // finally we close the unsed read end
         close(pipefc[1]); //close used
    }/*
    else if(pid2 == 0){
         close(pipefc[1]);
         read(pipefc[0],done,sizeof(done));
         close(pipefc[0]);
         printf("szülő olvasta a gyerek valasszat pipefc-n\n");
    }*/
    else
    {    // szulo process
         printf("Szulo indul!\n");
         close(pipefd[0]); //Usually we close unused read end            //lezárjuk az olvasó véget
      close(pipefc[1]);            //amit nem haszn
         //write(pipefd[1], "Hajra Fradi!",13);                          //ezt akarjuk eljküldeni a csővezetékben, karakterszám + végkarakter
         //write(pipefd[1], currentJob[0], sizeof(currentJob[0]));       //ezt akarjuk eljküldeni a csővezetékben, karakterszám + végkarakter
         printf("current nr: %d\n", current_nr);
         if(write(pipefd[1], current_nr, sizeof(current_nr)) >0){               //ezt akarjuk eljküldeni a csővezetékben, karakterszám + végkarakter
            printf("Szulo beirta az adatokat a csobe!\n");
         } else {
          printf("Szulo nem tud írni\n");
          exit(EXIT_FAILURE);
          exit(EXIT_FAILURE);
         }
         ...
         pid2 = fork();
         wait();  
         if(read(pipefc[0],done,sizeof(done))>0){
            printf("szülő olvasta a gyerek valasszat pipefc-n\n");
         } else{
            printf("szülő nem tudja olvasni a választ!\n");
            exit(EXIT_FAILURE);
            exit(EXIT_FAILURE);
         }
         close(pipefc[0]);
...
//    current = malloc(sizof(*current));
   ...
   while(option != 0){
   	...
   	if(option == 1){getNewOrder(&current);}
        ...
        if(option == 5){orderManager(&current);}
