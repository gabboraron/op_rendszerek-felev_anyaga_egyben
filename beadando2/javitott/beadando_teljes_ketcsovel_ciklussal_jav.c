...
int there_is_more = 0;      //is it there more jobs
void optimize(struct order *current){ 
  ...
  //get the oldest value  
	printf("\nsatatus \t name \t email \t phonenumber \t size \t systime\n");
  while(fread(obj, sizeof(struct order), 1, file)){  
	  if(isThisDone(obj) == 1){//this job is done
      printf("DONE \t %s \t %s \t %d \t %d \t %ld\n", obj->name, obj->email, obj->phonenumber, obj->size, obj->systime);
      ++nr_done;
    } else {//this job is active
      printf("ACTIVE \t %s \t %s \t %d \t %d \t %ld\n", obj->name, obj->email, obj->phonenumber, obj->size, obj->systime);
...
//      printf("ACTIVE \t %s \t %s \t %d \t %d \t %ld\n", obj->name, obj->email, obj->phonenumber, obj->size, obj->systime);
  ...
  if(idx>0)
  {
    ...
    //for( idx = 1; idx < size; idx = idx + 1 ){
      ...
      int jdx;
      printf("idx: %d\n",idx);
      for( jdx = idx+1; jdx < size; jdx = jdx + 1 ){//get the second with this size
        printf("\t jdx: %d\n",jdx);
...
int isThisDone(struct order *current);
int isThisDone(struct order *current){
  struct order *obj=malloc(sizeof(struct order));
  FILE * file = fopen("done_db.txt", "rb");
  fseek(file, 0, SEEK_SET);
  ...
  while(fread(obj,sizeof(struct order), 1, file)){
    //checkValues(obj);
    if((strcmp(obj->name, current->name) == 0) && (strcmp(obj->email, current->email) == 0)){
      //printf("DONE by isThisDone \t %s/%s/%d/%d\n", obj->name, obj->email, obj->phonenumber, obj->size);
      fclose(file);
      return 1;
    }
  }
...
//set job as done
void set_as_done(char *s, struct order *current);
void set_as_done(char *s, struct order *current) {
  s[0] = 'O';
  s[1] = 'K'; 
  //set into done db
  FILE * file=fopen("done_db.txt", "a");
  if(file != NULL){
    fwrite(current, sizeof(struct order), 1, file);
    fclose(file);
  }
...
//signal handler, run when child's signal has arrived
void handler(int signumber);
void handler(int signumber){
  printf("Signal with number %i has arrived\n",signumber);
}

//send orders to childe, and optimaze them
void orderManager(struct order *current){
  optimize(current);
  int idx = 0;
  printf("\n \t *MANAGE JOBS* \n");
...
printf("there_is_more:  %d\n", there_is_more);
  while(there_is_more <1){
    for (idx = 0; idx < nrOfJobs; ++idx)
    {
      ...
      char original[80]; 
      strcpy(original, currentJob[idx].email);
      char buf[80];
      int  p1[2], p2[2];
